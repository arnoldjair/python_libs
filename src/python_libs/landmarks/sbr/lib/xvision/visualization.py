...
from python_libs.landmarks.sbr.lib.datasets import pil_loader
...
def draw_image_by_points(_image, pts, radius, color, crop, resize):
    ...
    assert isinstance(pts, np.ndarray) and (
        pts.shape[0] == 2 or pts.shape[0] == 3
    ), "input points are not correct"

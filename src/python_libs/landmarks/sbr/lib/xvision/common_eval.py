...
def evaluate_normalized_mean_error(predictions, groundtruth, log, extra_faces):
    ...
    if extra_faces is not None:
        assert len(extra_faces) == len(
            predictions
        ), "The length of extra_faces is not right {} vs {}".format(
    ...
    for i in range(num_images):
        ...
        if num_points == 68:
            ...
            interocular_distance = np.linalg.norm(
                ground_truth_points[:2, 19] - ground_truth_points[:2, 28]
            )
        ...
        for j in range(num_points):
            if bool(ground_truth_points[2, j]):
                dis_sum = dis_sum + np.linalg.norm(
                    detected_points[:2, j] - ground_truth_points[:2, j]
                )
                pts_sum = pts_sum + 1

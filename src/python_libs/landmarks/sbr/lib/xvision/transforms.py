...
class TrainScale2WH(object):
    ...
    def __init__(self, target_size, interpolation=Image.BILINEAR):
        assert isinstance(target_size, tuple) or isinstance(
            target_size, list
        ), "The type of target_size is not right : {}".format(target_size)
        assert (
            len(target_size) == 2
        ), "The length of target_size is not right : {}".format(target_size)
        assert isinstance(target_size[0], int) and isinstance(
            target_size[1], int
        ), "The type of target_size is not right : {}".format(target_size)
    ...
    def __call__(self, imgs, point_meta):
        ...
        point_meta.apply_scale([ow * 1.0 / w, oh * 1.0 / h])
...
class ToTensor(object):
    ...
    def __call__(self, pics, points):
        ...
        if is_list == False:
            assert (
                len(returned) == 1
            ), "For non-list data, length of answer must be one not {}".format(
                len(returned)
            )
            returned = returned[0]
...
class PreCrop(object):
    ...
    def __init__(self, expand_ratio):
        assert expand_ratio is None or isinstance(
            expand_ratio, numbers.Number
        ), "The expand_ratio should not be {}".format(expand_ratio)
        if expand_ratio is None:
            self.expand_ratio = 0
        else:
            self.expand_ratio = expand_ratio
        assert self.expand_ratio >= 0, "The expand_ratio should not be {}".format(
            expand_ratio
        )
    ...
    def __call__(self, imgs, point_meta):
        ...
        face_ex_w, face_ex_h = (box[2] - box[0]) * self.expand_ratio, (
            box[3] - box[1]
        ) * self.expand_ratio
        x1, y1 = int(max(math.floor(box[0] - face_ex_w), 0)), int(
            max(math.floor(box[1] - face_ex_h), 0)
        )
        x2, y2 = int(min(math.ceil(box[2] + face_ex_w), w)), int(
            min(math.ceil(box[3] + face_ex_h), h)
        )
...
class AugScale(object):
    ...
    def __init__(self, scale_prob, scale_min, scale_max, interpolation=Image.BILINEAR):
        assert (
            isinstance(scale_prob, numbers.Number) and scale_prob >= 0
        ), "scale_prob : {:}".format(scale_prob)
        assert isinstance(scale_min, numbers.Number) and isinstance(
            scale_max, numbers.Number
        ), "scales : {:}, {:}".format(scale_min, scale_max)
...
class AugCrop(object):
    def __init__(self, crop_x, crop_y, center_perterb_max, fill=0):
        assert (
            isinstance(crop_x, int)
            and isinstance(crop_y, int)
            and isinstance(center_perterb_max, numbers.Number)
        )
        self.crop_x = crop_x
        self.crop_y = crop_y
    ...
    def __call__(self, imgs, point_meta=None):
        ...
        x1 = int(round(point_meta.center[0] + x_offset - self.crop_x / 2.0))
        y1 = int(round(point_meta.center[1] + y_offset - self.crop_y / 2.0))
        x2 = x1 + self.crop_x
        y2 = y1 + self.crop_y

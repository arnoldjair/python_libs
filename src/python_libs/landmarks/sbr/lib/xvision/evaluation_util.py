...
from python_libs.landmarks.sbr.lib.log_utils import print_log
...
class Eval_Meta:
    ...
    def __repr__(self):
        ...
        ) + "(number of data = {:})".format(len(self))
    ...
    def append(self, _pred, _ground, image_path, face_size):
        assert (
            _pred.shape[0] == 3 and len(_pred.shape) == 2
        ), "Prediction's shape is {:} vs [should be (3,pts) or (2,pts)]".format(
            _pred.shape
        )
        if _ground is not None:
            assert (
                _pred.shape == _ground.shape
            ), "shapes must be the same : {} vs {}".format(_pred.shape, _ground.shape)
        if (not self.predictions) == False:
            assert (
                _pred.shape == self.predictions[-1].shape
            ), "shapes must be the same : {} vs {}".format(
                _pred.shape, self.predictions[-1].shape
            )
    ...
    def compute_mse(self, log):
        ...
        for x, gt, face in zip(self.predictions, self.groundtruth, self.face_sizes):
            ...
            num += 1
        print_log(
            "Filter the unlabeled data from {:} into {:} data".format(len(self), num),
            log,
        )
        if num == 0:
            nme, auc, pck_curves = -1, None, None
        else:
            nme, auc, pck_curves = evaluate_normalized_mean_error(
                self.predictions, self.groundtruth, log, self.face_sizes
            )

...
from python_libs.landmarks.sbr.lib.pts_utils import generate_label_map

from .dataset_utils import anno_parser, pil_loader
...
class VideoDataset(data.Dataset):
    def __init__(
        self, transform, sigma, downsample, heatmap_type, data_indicator, video_parser
    ):
    ...
    def __repr__(self):
        return "{name}(point-num={NUM_PTS}, sigma={sigma}, heatmap_type={heatmap_type}, length={length}, dataset={dataset_name}, parser={video_parser})".format(
    ...
    def append(self, data, label, box, face_size):
        ...
        if (label is not None) and (label.lower() != "none"):
            if isinstance(label, str):
                assert osp.isfile(
                    label
                ), "The annotation path is not a file : {}".format(label)
    ...
    def load_data(self, datas, labels, boxes, face_sizes, num_pts, reset):
        ...
        assert isinstance(
            datas, list
        ), "The type of the datas is not correct : {}".format(type(datas))
        assert isinstance(labels, list) and len(datas) == len(
            labels
        ), "The type of the labels is not correct : {}".format(type(labels))
        assert isinstance(boxes, list) and len(datas) == len(
            boxes
        ), "The type of the boxes is not correct : {}".format(type(boxes))
        assert isinstance(face_sizes, list) and len(datas) == len(
            face_sizes
        ), "The type of the face_sizes is not correct : {}".format(type(face_sizes))
        if reset:
            self.reset(num_pts)
        else:
            assert (
        ...
        for idx, data in enumerate(datas):
            assert isinstance(data, str), "The type of data is not correct : {}".format(
                data
            )
        ...
        assert (
            len(self.face_sizes) == self.length
        ), "The length and the face_sizes is not right {} vs {}".format(
            self.length, len(self.face_sizes)
        )
        print(
            "Load data done for the general dataset, which has {} images.".format(
                self.length
            )
        )
    ...
    def load_list(self, file_lists, num_pts, reset):
        ...
        for idx, data in enumerate(lists):
            ...
            assert (
                len(alls) == 6 or len(alls) == 7
            ), "The {:04d}-th line in {:} is wrong : {:}".format(idx, data)
            datas.append(alls[0])
            if alls[1] == "None":
                labels.append(None)
            else:
            ...
            box = np.array(
                [float(alls[2]), float(alls[3]), float(alls[4]), float(alls[5])]
            )
            boxes.append(box)
            if len(alls) == 6:
    ...
    def __getitem__(self, index):
        ...
        if target.is_none() == False:
            ...
            points = torch.from_numpy(np.zeros((self.NUM_PTS, 3))).type(
                torch.FloatTensor
            )

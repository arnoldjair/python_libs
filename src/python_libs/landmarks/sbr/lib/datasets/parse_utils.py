...
def parse_basic(ori_filename, length_l, length_r):
    ...
    if folder[-10:] == "extraction":
        assert filename[-4:] == ".png", "The filename is not right : {}".format(
            filename
        )
        ...
        for i in range(idx - length_l, idx + length_r + 1):
            ...
            assert osp.isfile(
                path
            ), "!!WRONG file path : {}, the original frame is {}".format(path, filename)
            images.append(path)
        return images, True
    # YouTube Cele..
    ...
    # Talking Face..
        ...
        assert filename[-4:] == ".jpg", "The filename is not right : {}".format(
            filename
        )
    ...
    # YouTube Face..
        ...
        assert filename[:5] == "image", "The filename is not right : {}".format(
            filename
        )
        splits = filename.split(".")
...
def parse_video_by_indicator(image_path, parser, return_info=False):
    if parser is None or parser.lower() == "none":
        ...
        parser = parser.split("-")
        assert len(parser) == 3, "The video parser must be 3 elements : {:}".format(
            parser
        )

...
from python_libs.landmarks.sbr.lib.utils import load_txt_file
...
def anno_parser_v0(anno_path, num_pts):
    ...
    assert (
        data[1].find("n_points: ") == 0
    ), "number of points in second line is not correct"
    assert data[2] == "{" and data[-1] == "}", "starting and end symbol is not correct"

    assert (
        data[0] == "version: 1" or data[0] == "version: 1.0"
    ), "The version is wrong : {}".format(data[0])
    n_points = int(data[1][len("n_points: ") :])

    assert (
        num_lines == n_points + 4
    ), "number of lines is not correct"  # 4 lines for general information: version, n_points, start and end symbol
    ...
    for point_index in range(n_points):
        try:
            ...
            if (
                len(pts_list) > 2
            ):  # handle edge case where additional whitespace exists after point coordinates
            ...
            pts[2, point_index] = float(
                1
            )  # oculusion flag, 0: oculuded, 1: visible. We use 1 for all points since no visibility is provided by 300-W
    ...
    for line in data:
        try:
            ...
            if one_base == False:
                idx = idx + 1
            assert (
                idx >= 1 and idx <= NUM_PTS
            ), "Wrong idx of points : {:02d}-th in {:s}".format(idx, anno_path)
            pts[0, idx - 1] = point_x
            pts[1, idx - 1] = point_y
...
def resize_heatmap(maps, height, width, order=3):
    ...
    # order = 2 Cubic
    assert (
        isinstance(maps, np.ndarray) and len(maps.shape) == 3
    ), "maps type : {}".format(type(maps))
...
def merge_lists_from_file(file_paths, seed=None):
    ...
    print(
        "merge lists from {} files with seed={} for random shuffle".format(
            len(file_paths), seed
        )
    )
    # load the data

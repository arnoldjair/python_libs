...
class Point_Meta:
    ...
    def __init__(self, num_point, points, box, image_path, dataset_name):
        ...
        ), "The shape of box is not right : {}".format(box)
        self.box = box.copy()
        if points is None:
            self.points = points
        else:
    ...
    def __repr__(self):
        return "{name}(number of point={num_point})".format(
    ...
    def apply_scale(self, scale):
        if len(scale) == 1:  # scale the same size for both x and y
            if self.points is not None:
                self.points[:2, self.points[2, :] > 0] = (
                    self.points[:2, self.points[2, :] > 0] * scale[0]
                )
            ...
            if self.points is not None:
                self.points[0, self.points[2, :] > 0] = (
                    self.points[0, self.points[2, :] > 0] * scale[0]
                )
                self.points[1, self.points[2, :] > 0] = (
                    self.points[1, self.points[2, :] > 0] * scale[1]
                )
    ...
    def apply_offset(self, ax=None, ay=None):
        if ax is not None:
            if self.points is not None:
                self.points[0, self.points[2, :] > 0] = (
                    self.points[0, self.points[2, :] > 0] + ax
                )
        ...
        if ay is not None:
            if self.points is not None:
                self.points[1, self.points[2, :] > 0] = (
                    self.points[1, self.points[2, :] > 0] + ay
                )
    ...
    def apply_rotate(self, center, degree):
        ...
        if self.points is not None:
            ...
            self.points[0, self.points[2, :] > 0] = (
                (vis_xs - center[0]) * np.cos(degree)
                - (vis_ys - center[1]) * np.sin(degree)
                + center[0]
            )
            self.points[1, self.points[2, :] > 0] = (
                (vis_xs - center[0]) * np.sin(degree)
        ...
        corners[:, 0] = (
            (corners[:, 0] - center[0]) * np.cos(degree)
            - (corners[:, 1] - center[1]) * np.sin(degree)
            + center[0]
        )
        corners[:, 1] = (
            (corners[:, 0] - center[0]) * np.sin(degree)
    ...
    def apply_horizontal_flip(self, width):
        ...
        if self.datasets == "Mugsy.full_face_v1":
            ...
            pos = np.array(
                [3, 4, 1, 2, 9, 10, 11, 12, 5, 6, 7, 8, 14, 13, 15, 16, 17, 18, 19, 20]
            )

...
def generate_label_map(pts, height, width, sigma, downsample, nopoints, ctype):
    ...
    assert (
        isinstance(pts, np.ndarray) and len(pts.shape) == 2 and pts.shape[0] == 3
    ), "The shape of points : {}".format(pts.shape)
    if isinstance(sigma, numbers.Number):
        sigma = np.zeros((pts.shape[1])) + sigma
    assert (
        isinstance(sigma, np.ndarray)
        and len(sigma.shape) == 1
        and sigma.shape[0] == pts.shape[1]
    ), "The shape of sigma : {}".format(sigma.shape)
    ...
    transformed_label = np.fromfunction(
        lambda y, x, pid: (
            (offset + x * downsample - pts[0, pid]) ** 2
            + (offset + y * downsample - pts[1, pid]) ** 2
        )
        / -2.0
        / sigma[pid]
        / sigma[pid],
        (height, width, num_points),
        dtype=int,
    )
    ...
    if ctype == "laplacian":
        ...
        raise TypeError(
            "Does not know this type [{:}] for label generation".format(ctype)
        )
    ...
    heatmap = np.concatenate(
        (transformed_label, np.expand_dims(background_label, axis=2)), axis=2
    ).astype("float32")

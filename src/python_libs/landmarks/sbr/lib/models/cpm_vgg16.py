...
class VGG16_base(nn.Module):
    def __init__(self, config, pts_num):
        ...
        for i in range(1, self.config.stages):
            stagex = nn.Sequential(
                nn.Conv2d(128 + pts_num, 128, kernel_size=7, dilation=1, padding=3),
                nn.ReLU(inplace=True),
    ...
    def specify_parameter(self, base_lr, base_weight_decay):
        params_dict = [
            {
                "params": get_parameters(self.features, bias=False),
                "lr": base_lr,
                "weight_decay": base_weight_decay,
            },
            {
                "params": get_parameters(self.features, bias=True),
                "lr": base_lr * 2,
                "weight_decay": 0,
            },
            {
                "params": get_parameters(self.CPM_feature, bias=False),
                "lr": base_lr,
                "weight_decay": base_weight_decay,
            },
            {
                "params": get_parameters(self.CPM_feature, bias=True),
                "lr": base_lr * 2,
                "weight_decay": 0,
            },
        ]
        for stage in self.stages:
            params_dict.append(
                {
                    "params": get_parameters(stage, bias=False),
                    "lr": base_lr * 4,
                    "weight_decay": base_weight_decay,
                }
            )
            params_dict.append(
                {
                    "params": get_parameters(stage, bias=True),
                    "lr": base_lr * 8,
                    "weight_decay": 0,
                }
            )
    ...
    def forward(self, inputs):
        assert (
            inputs.dim() == 4
        ), "This model accepts 4 dimension input tensor: {}".format(inputs.size())
        ...
        for ibatch in range(batch_size):
            batch_location, batch_score = find_tensor_peak_batch(
                batch_cpms[-1][ibatch], self.config.argmax, self.downsample
            )

...
class LK(nn.Module):
    ...
    def forward(self, inputs):
        assert (
            inputs.dim() == 5
        ), "This model accepts 5 dimension input tensor: {}".format(inputs.size())
        ...
        batch_locs, batch_scos = batch_locs.view(
            batch_size, sequence, self.points, 2
        ), batch_scos.view(batch_size, sequence, self.points)
        batch_next, batch_fback, batch_back = [], [], []

        for ibatch in range(batch_size):
            feature_old = inputs[ibatch]
            nextPts, fbackPts, backPts = lk.lk_forward_backward_batch(
                inputs[ibatch],
                batch_locs[ibatch],
                self.config.window,
                self.config.steps,
        ...
        batch_next, batch_fback, batch_back = (
            torch.stack(batch_next),
            torch.stack(batch_fback),
            torch.stack(batch_back),
        )

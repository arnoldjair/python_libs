...
def torch_inverse(deltp):
    assert (
        deltp.dim() == 2 and deltp.size(0) == 2 and deltp.size(1) == 2
    ), "The deltp format is not right : {}".format(deltp.size())
...
class SobelConv(nn.Module):
    def __init__(self, tag, dtype):
        ...
        if tag == "x":
            Sobel = np.array(
                [[-1.0 / 8, 0, 1.0 / 8], [-2.0 / 8, 0, 2.0 / 8], [-1.0 / 8, 0, 1.0 / 8]]
            )
            # Sobel = np.array([ [ 0, 0, 0], [-0.5,0,0.5], [ 0, 0, 0] ])
        elif tag == "y":
            Sobel = np.array(
                [[-1.0 / 8, -2.0 / 8, -1.0 / 8], [0, 0, 0], [1.0 / 8, 2.0 / 8, 1.0 / 8]]
            )
...
def ComputeGradient(feature, tag):
    if feature.dim() == 3:
        ...
        squeeze = False
    assert feature.dim() == 4, "feature must be [batch x C x H x W] not {}".format(
        feature.size()
    )
    sobel = SobelConv(tag)
    if feature.is_cuda:
...
def Generate_Weight(patch_size, sigma=None):
    ...
    assert (
        patch_size[0] > 0 and patch_size[1] > 0
    ), "the patch size must > 0 rather :{}".format(patch_size)
    center = [(patch_size[0] - 1.0) / 2, (patch_size[1] - 1.0) / 2]
    maps = np.fromfunction(
        lambda x, y: (x - center[0]) ** 2 + (y - center[1]) ** 2,
        (patch_size[0], patch_size[1]),
        dtype=int,
    )
    if sigma is None:
        sigma = min(patch_size[0], patch_size[1]) / 2.0
...
def warp_feature(feature, pts_location, patch_size):
    ...
    affine_parameter = [
        ...
        MU.np2variable(torch.zeros(1), feature.is_cuda, False),

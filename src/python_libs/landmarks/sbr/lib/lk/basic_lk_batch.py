...
from .basic_utils import Generate_Weight, SobelConv
...
def lk_tensor_track_batch(
    feature_old, feature_new, pts_locations, patch_size, max_step, feature_template=None
):
    ...
    assert (
        feature_new.dim() == 4
    ), "The dimension of feature-new is not right : {}.".format(feature_new.dim())
    BB, C, H, W = list(feature_new.size())
    if feature_old is not None:
        assert (
            1 == feature_old.size(0) and 1 == BB
        ), "The first dimension of feature should be one not {}".format(
            feature_old.size()
        )
        assert (
            C == feature_old.size(1)
            and H == feature_old.size(2)
            and W == feature_old.size(3)
        ), "The size is not right : {}".format(feature_old.size())
    assert isinstance(
        patch_size, int
    ), "The format of lk-parameters are not right : {}".format(patch_size)
    ...
    with torch.no_grad():
        weight_map = (
            ...
            .view(1, 1, 1, patch_size * 2 + 1, patch_size * 2 + 1)
            .to(device)
        )
    ...
    if feature_template is None:
        ...
        assert (
            feature_old is None
        ), "When feature_template is not None. feature_old must be None"
        feature_T = feature_template
    assert (
        feature_T.size(2) == patch_size * 2 + 1
        and feature_T.size(3) == patch_size * 2 + 1
    ), "The size of feature-template is not ok : {}".format(feature_T.size())
    ...
    H = torch.bmm(
        weightedJ.view(num_pts, 2, -1), J.view(num_pts, 2, -1).transpose(2, 1)
    )
...
def lk_forward_backward_batch(features, locations, window, steps):
    ...
    for iseq in range(1, sequence):
        ...
        nextPts = lk_tensor_track_batch(
            feature_old, feature_new, previous_pts[iseq - 1], window, steps, None
        )
    ...
    for iseq in range(sequence - 2, -1, -1):
        feature_old = features.narrow(0, iseq + 1, 1)
        feature_new = features.narrow(0, iseq, 1)
        backPts = lk_tensor_track_batch(
            feature_old, feature_new, fback_pts[iseq + 1], window, steps, None
        )
    ...
    for iseq in range(sequence - 2, -1, -1):
        ...
        feature_new = features.narrow(0, iseq, 1)
        backPts = lk_tensor_track_batch(
            feature_old, feature_new, back_pts[iseq + 1], window, steps, None
        )

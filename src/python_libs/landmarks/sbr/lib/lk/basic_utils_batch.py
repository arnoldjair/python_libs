...
def torch_inverse_batch(deltp):
    # deltp must be [K,2]
    assert (
        deltp.dim() == 3 and deltp.size(1) == 2 and deltp.size(2) == 2
...
def warp_feature_batch(feature, pts_location, patch_size):
    ...
    assert (
        ...
        and feature.size(0) == 1
        and pts_location.size(1) == 2
    ), "The shapes of feature or points are not right : {} vs {}".format(
        feature.size(), pts_location.size()
    )

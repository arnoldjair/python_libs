...
import numpy as np
import torch
from log_utils import AverageMeter, convert_secs2time, time_for_file
...
def basic_train(args, loader, net, criterion, optimizer, epoch_str, logger, opt_config):
    ...
    for i, (
        ...
        np_batch_locs, np_batch_scos = (
            batch_locs.detach().to(cpu).numpy(),
            batch_scos.detach().to(cpu).numpy(),
        )
        ...
        for ibatch, (imgidx, nopoint) in enumerate(zip(image_index, nopoints)):
            ...
            ), "The ibatch={:}, imgidx={:} is not right.".format(
        ...
        if i % args.print_freq == 0 or i + 1 == len(loader):
            logger.log(
                ...
                + last_time
                + show_stage_loss(each_stage_loss_value)

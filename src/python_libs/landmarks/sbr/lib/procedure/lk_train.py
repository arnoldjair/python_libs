...
def lk_train(
    ...
    for i, (
        ...
        if annotated_num > 0:
            ...
            detloss, each_stage_loss_value = compute_stage_loss(
                criterion, target, annot_heatmaps, mask
            )
            if opt_config.lossnorm:
                detloss, each_stage_loss_value = detloss / annotated_num / 2, [
        ...
        if use_lk:
            lkloss, avaliable = lk_target_loss(
                batch_locs,
                batch_scos,
                batch_next,
                batch_fback,
        ...
        if isinstance(loss, numbers.Number):
            warnings.warn(
                "The {:}-th iteration has no detection loss and no lk loss".format(i)
            )
        else:

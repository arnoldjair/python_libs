...
def lk_input_check(batch_locs, batch_scos, batch_next, batch_fback, batch_back):
    ...
    assert (
        batch_scos.size(0) == batch
        and batch_scos.size(1) == sequence
        and batch_scos.size(2) == num_pts
    ), "{:} vs {:}".format(batch_locs.size(), batch_scos.size())
...
def lk_target_loss(
    ...
    batch, sequence, num_pts = lk_input_check(
        batch_locs, batch_scos, batch_next, batch_fbak, batch_back
    )
    ...
    for ibatch in range(batch):
        for ipts in range(num_pts):
            if not sequence_checks[ibatch, ipts]:
                continue
            loss = 0
            for iseq in range(sequence):
                ...
                if (
                    fbak_distance.item() > lk_config.fb_thresh
                    or fbak_distance.item() < lk_config.eps
                ):  # forward-backward-check
                    if iseq + 1 < sequence:
                        sequence_checks[ibatch, ipts] = False
                if (
                    forw_distance.item() > lk_config.forward_max
                    or forw_distance.item() < lk_config.eps
                ):  # to avoid the tracker point is too far
                    if iseq > 0:
                        sequence_checks[ibatch, ipts] = False
                if (
                    back_distance.item() > lk_config.forward_max
                    or back_distance.item() < lk_config.eps
                ):  # to avoid the tracker point is too far
                    if iseq + 1 < sequence:
                        sequence_checks[ibatch, ipts] = False

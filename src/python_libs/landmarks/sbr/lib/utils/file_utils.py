...
def load_list_from_folder(folder_path, ext_filter=None, depth=1):
    ...
    assert (
        ext_filter is None
        or (
            isinstance(ext_filter, list)
            and all(isinstance(ext_tmp, str) for ext_tmp in ext_filter)
        )
        or isinstance(ext_filter, str)
...
def load_list_from_folders(folder_path_list, ext_filter=None, depth=1):
    ...
    """
    assert isinstance(folder_path_list, list) or isinstance(
        folder_path_list, str
    ), "input path list is not correct"
    ...
    for folder_path_tmp in folder_path_list:
        fulllist_tmp, num_elem_tmp = load_list_from_folder(
            folder_path_tmp, ext_filter=ext_filter, depth=depth
        )

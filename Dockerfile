...
# and `pip install python_libs` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "python_libs", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n python_libs -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "python_libs", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "python_libs", "YOUR_CONSOLE_SCRIPT"]
